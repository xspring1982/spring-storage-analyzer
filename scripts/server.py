...
- Application folders outside $HOME may only be opened in the file manager for
  manual review or uninstall. They are never valid roots for rm/trash actions.
...
def allowed_roots_for_mode(mode):
    if mode != "open":
        return (HOME,)
    roots = [HOME]
    if sys.platform == "darwin":
...
def path_is_within_allowed_roots(path, mode):
    return any(path == base or path.startswith(base + os.sep)
               for base in allowed_roots_for_mode(mode))
    ...
    def do_POST(self):
        ...
        for p in (req.get("paths") or []):
            ...
            if rp not in allow:
                ...
                return
            # 二级护栏：删除/移废纸篓只允许用户目录；应用目录仅允许 open。
            if not path_is_within_allowed_roots(rp, mode):
